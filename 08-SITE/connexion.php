...
if (isset($_GET['action']) && $_GET['action'] == 'deconnexion') { // si le membre a cliqué sur "deconnexion"
...
// 3 - Le membre deja connecté et redirigé vers son profil
if(estConnecte()){
...
if (!empty($_POST)){ //si le formulaire a été envoyé
        ...
        if(empty($contenu)) { // si c'est vide c'est qu'il n'y a pas d'erreur
            ...
            if($resultat -> rowCount()==1){ // si il y a 1 ligne de résultat alors le pseudo existe  : on peut vérifier le MDP
                $membre = $resultat->fetch(PDO::FETCH_ASSOC); // on "fetche" l'object $resultat pour en extraire les données du membre (pas de boucle) car le pseudo est unique
                ...
                } else { // sinon dès que les mdp ne correspond pas 
