...
if(!empty($_POST)){ // si $_POST n'est pas vide c'est que le formulaire a été envoyé
    // validation du formulaire :
        if (!isset($_POST['pseudo']) || strlen($_POST['pseudo']) < 4 || strlen($_POST['pseudo'])  > 20) { // si le pseudo n'existe pas dans $_POST c'est que le formulaire est altéré, ou la valeur est < 4 ou la valeur est > 20 (pour la BDD), on met un message d'errur à l'internaute
...
if ($affiche_formulaire):  // syntaxe en if ():  ....endif oû le ":" correspond à l'accolade ouvrante et le endif à la fermante. Si le membre n'est pas inscrit on lui affiche le formulaire.
?>
